class DetailServiceGroup < ActiveRecord::Base
	...
	def print
		File.open("C:/prueba/tedef/"+self.code+"/"+self.name, 'w:ANSI') do |f| 
	...
	def get_line(ds)
		...
		tuition_code = ds.tuition_code.rjust(6,' ')
	 	...
	 	return clinic_ruc+clinic_code+payment_type_document+payment_document+correlative_benefit+correlative+clasification_code+service_code+service_description+date+professional_type+tuition_code+quantity+unitary+copayment+amount+amount_not_covered+diagnostic+exented_code+sector_code
	end
end
